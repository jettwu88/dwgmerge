...
def run_merge(a_doc: ezdxf.document.Drawing,
    ...
    # ------------------------------------------------------------
    # Guard: the "move B's old furniture by a single offset, then
    # delete whatever now exactly overlaps A's new furniture" strategy
    # below only works when A's target paper size is (approximately)
    # the same as B's OWN original paper size - a uniform translation
    # can correctly align two frames of the same size, but not two
    # differently-sized frames anchored at different corners. If the
    # sizes differ substantially, the "supersede B's duplicate old
    # furniture" step below silently finds zero matches, and ALL of
    # B's old border/table/notes furniture is left behind untouched,
    # duplicating everything inside the new frame. Fail clearly here
    # instead of silently shipping that broken result (this is exactly
    # what happened when "A3" was picked as the target for a drawing
    # that is itself A4-sized).
    # ------------------------------------------------------------
    _PAPER_SIZE_TOLERANCE_MM = 5.0
    b_orig_w = b_layout.dxf.paper_width
    b_orig_h = b_layout.dxf.paper_height
    a_dims = sorted([a_layout.dxf.paper_width, a_layout.dxf.paper_height])
    b_dims = sorted([b_orig_w, b_orig_h])
    if (abs(a_dims[0] - b_dims[0]) > _PAPER_SIZE_TOLERANCE_MM
            or abs(a_dims[1] - b_dims[1]) > _PAPER_SIZE_TOLERANCE_MM):
        raise ValueError(
            f"B 圖本身的圖紙尺寸是 {b_orig_w:.0f} x {b_orig_h:.0f} mm，"
            f"但你選的目標圖紙尺寸 「{target_layout_name}」 是 "
            f"{a_layout.dxf.paper_width:.0f} x {a_layout.dxf.paper_height:.0f} mm，"
            "兩者尺寸差太多。這個工具目前的合併演算法只支援目標尺寸跟 B 原本尺寸相近的情況"
            "（否則 B 原有的邊框/表格會沒被正確取代，整張圖出現重複內容）。"
            "請改選跟 B 原本圖紙尺寸相符的圖紙尺寸再試一次。"
        )
    ...
    # Make the OUTPUT layout's name match what was actually requested -
    # B's own layout keeps its original name (e.g. "A4") by default,
    # which is misleading (and inaccessible by the requested name) when
    # the user picked a differently-named-but-same-size target layout.
    if b_layout_name != target_layout_name:
        try:
            if target_layout_name in core.list_layout_names(dst_doc):
                # A layout with that name already exists in B itself
                # (e.g. B has both "A4" and "A3" sheets) - don't clobber
                # it, leave the naming as-is and just warn.
                warnings.append(
                    f"輸出的圖紙仍叫做「{b_layout_name}」，因為 B 圖裡已經有一個叫"
                    f"「{target_layout_name}」的圖紙，避免同名衝突。"
                )
            else:
                dst_doc.layouts.rename(b_layout_name, target_layout_name)
        except Exception as e:
            warnings.append(f"輸出圖紙重新命名為「{target_layout_name}」失敗，仍維持原名。({e})")
    ...
    if ole_objects:
        ...
        # No BOM-like object in this drawing, but the same rule still
        # applies: "不管是圖框內外的資訊都不能重疊" is universal, not
        # conditional on there being a BOM table to share the sheet
        # with. A plain part drawing's (possibly B-extended) Notes
        # block can just as easily end up overlapping the title block's
        # own field cluster or other frame furniture once combined -
        # confirmed on 871203 - so check/relocate it exactly the same
        # way as the BOM-object path, rather than the old weaker
        # "nudge up, warn if it still overlaps" behaviour.
        _relocate_outside_if_overlapping(
            notes_entity, protected_boxes, viewport_boxes,
            dst_layout.dxf.paper_width, dst_layout.dxf.paper_height, warnings,
            label="備註 (Notes)", outside_boxes=outside_boxes)
